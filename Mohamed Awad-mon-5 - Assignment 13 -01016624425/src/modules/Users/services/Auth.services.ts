...
import { compareHash, createAndSendOTP, encrypt, generatehHash, redis, SuccessResponse } from "../../../utils";
...
private userRepo:UserRepo=new UserRepo(UserModel)
      SignUp =async(req:Request,res:Response)=>{
        ...
        const checkEmail=await this.userRepo.findOneDocument({email:signUpDTO.email} )
        if(checkEmail)throw new conflictException(`email is already exist`)
          const hashPassword=generatehHash(signUpDTO.password)
            const created=await this.userRepo.createDocument({...signUpDTO,password:hashPassword,phone:encrypt(signUpDTO.phone)})
            await createAndSendOTP(signUpDTO.email)
        return res.json(SuccessResponse(`user created`,201,created))
   }
    confrim_email = async (req:Request, res:Response) => {
  const confirmDTO:confirmEmailDTO = req.body;
  const User = await this.userRepo.findOneDocument({email:confirmDTO.email})
  ...
  await User.save();
  return res.json(SuccessResponse(`emailis confirmed`,200));
}
 resendOTP =async (req:Request, res:Response) => {
  const email:string = req.query.email as string ;
  const User = await this.userRepo.findOneDocument({email:email,isConfirmed:false});
  if (!User) throw new notFoundException(`email not found or confimed`);
  await createAndSendOTP(User.email);
  return res.json(SuccessResponse(`OTP send`,200));
};
 loginuser =async (req:Request, res:Response) => {
  ...
  if (!user.isConfirmed)
    throw new BadRequestException(`email not verified please verify email first`,{});
  const passMatch =await compareHash(logindto.password,user.password as string);
  if (!passMatch) throw new BadRequestException(`invalid Password or email`);
  const accessToken =  await generateTokens({res:res, id:user._id as string,role: user.role as string});
