...
class Comment_services {
  ...
  getComments = async (req: Request, res: Response): Promise<Response> => {
    ...
    return res
      .status(200)
      .json(SuccessResponse("comments found", 200, { comments }));
  };
  ...
  createReply = async (req: Request, res: Response): Promise<Response> => {
    ...
    return res
      .status(201)
      .json(SuccessResponse("Reply created", 201, { created }));
  };
  updateComment = async (req: Request, res: Response): Promise<Response> => {
    const commentId = req.params
      .commentId as unknown as mongoose.Types.ObjectId;
    const commentDTO: updateCommentDTO = req.body;
    const commentupdated = await this.commentRepo.findAndUpdateDocument(
      commentId,
      { content: commentDTO.content, mentions: commentDTO.mention }
    );
    if (!commentupdated) throw new notFoundException("comment not found ");
    return res
      .status(200)
  ...
  Reaction = async (req: Request, res: Response): Promise<Response> => {
    ...
    if (existingReaction) {
      if (existingReaction.Reaction == reaction) {
        ...
        comment.reactionsCount = Number(comment.reactionsCount) - 1;
      } else {
        existingReaction.Reaction = reaction;
      }
    } else {
      comment.reactions?.push({ userId, Reaction: reaction });
      comment.reactionsCount = Number(comment.reactionsCount) + 1;
    }
  ...
  deleteComment = async (req: Request, res: Response): Promise<Response> => {
    ...
    if (!comment) throw new notFoundException("comment not found");
    const deleted = await this.commentRepo.deleteDocument(commentId);
    const childsDeleted = await this.commentRepo.deletManyDocuments({
      /// transaction
      parentId: commentId,
    });
    ...
    return res
      .status(200)
      .json(
        SuccessResponse("comment deleted", 200, { deleted, childsDeleted })
      );
  };
}
